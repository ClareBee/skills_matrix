...
**DOM XSS:**
- JS frameworks, SPAs, & APIs dynamically including attacker-controllable data to page = vulnerable to DOM XSS.
...
Escape w HTML entity encoding to prevent switching into any execution context, such as script/style/event handlers. Hex entities recommended. 5 characters significant in XML (&, <, >, ", '), & also forward slash, as helps end an HTML entity.
```html
 & --> &amp;
 < --> &lt;
 > --> &gt;
...
</script> closing tag will close a script block even tho it's inside a quoted string because HTML parser runs before JavaScript parser.
>Please note this is an aggressive escaping policy that over-encodes.
...
Don't put untrusted data into complex properties like url, behaviour, & custom (-moz-binding).
...
If app handles markup - untrusted input that is supposed to contain HTML - v difficult to validate. Encoding also difficult, as would break tags, so library to parse/clean HTML formatted text.
e.g. HtmlSanitizer, OWASP Java HTML Sanitizer, Ruby on Rails SanitizeHelper

7. **Avoid JavaScript URLs**
Untrusted URLs that include protocol js: will execute js code when used in URL DOM locations such as anchor tag HREF attributes or iFrame src locations. Validate all untrusted URLs to ensure only contain safe schemes e.g. HTTPS.
...
### DOM-based XSS
When browser rendering HTML & associated content like CSS, JavaScript, etc., identifies rendering contexts for different kinds of input & follows different rules for each.

Each parser has distinct & separate semantics & differing meanings & treatment of encoded values within each subcontext (HTML, HTML attribute, URL, and CSS) within the execution context.
...
When in DOM execution context, only need to JavaScript encode HTML attributes which do not execute code (attributes other than event handler, CSS, & URL attributes).
...
```
The `setAttribute(name_string,value_string)` method is dangerous because it implicitly coerces `value_string` into DOM attribute datatype of `name_string`.
...
```
Directly setting event handler attributes will allow JavaScript encoding to mitigate against DOM based XSS.
...
In order to mitigate against the CSS url() method, ensure that you are URL encoding the data passed to it
```javascript
